...
docker_login() {
  ...
  if [ -z "$server" ] || [ -z "$user" ] || [ -z "$password" ]; then
    report_warning "cuos:docker:bad_args" "docker_login called with empty args"
    return 1
  fi
  ...
  if ! tmp_output="$(jq --arg s "$server" --arg a "$auth" '
    ...
    report_err "cuos:docker:jq_merge_failed" "Failed to write auth for $server"
    return 1
  fi

  # Write back in-place without mv: truncate then write
  : > "$config_file" || { report_err "cuos:docker:write_failed" "Cannot truncate $config_file"; return 1; }
  printf '%s\n' "${tmp_output}" > "$config_file" || { report_err "cuos:docker:write_failed" "Cannot write $config_file"; return 1; }
  chmod 600 "$config_file"
